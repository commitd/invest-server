...
public class MonoAdapterTest {
  ...
  public void testConvertOutput() {
    ...
    when(resolutionEnvironment.convertOutput(ArgumentMatchers.any(String.class), ArgumentMatchers.any()))
  ...
  public void testConvertInput() {
    final AnnotatedType type = TypeFactory.parameterizedAnnotatedClass(List.class, new Annotation[0]);
    ...
    when(globalEnvironment.convertInput(ArgumentMatchers.any(String.class), ArgumentMatchers.any(),
        ArgumentMatchers.eq(valueMapper)))
            .then(invocation -> Optional.ofNullable(invocation.getArgument(0)));

    final MonoAdapter<String> adapter = new MonoAdapter<String>();
    final String original = "a";
    final Mono<String> output = adapter.convertInput(original, type, globalEnvironment, valueMapper);
