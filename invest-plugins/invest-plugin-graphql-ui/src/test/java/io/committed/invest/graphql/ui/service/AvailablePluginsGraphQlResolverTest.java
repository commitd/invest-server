...
public class AvailablePluginsGraphQlResolverTest {
  ...
  public void before() {
    ...
    final InvestUiExtensionRegistry uiRegistry = new InvestUiExtensionRegistry(Arrays.asList(
        new TestInvestUiExtension("plugin-1"),
        new TestInvestUiExtension("plugin-2")
  ...
  public void testUiPlugins() {
    final List<UiPlugin> uiPlugin = resolver.uiPlugins(new InvestServerNode()).collectList().block();
  ...
  public void testUiPluginMissing() {
    final Optional<UiPlugin> uiPlugin = resolver.uiPlugin(new InvestServerNode(), "plugin-none").blockOptional();
