...
public class PopulatingUserDetailsRepositoryAuthenticationManager
  ...
  public Mono<Authentication> authenticate(final Authentication authentication) {
    ...
    return this.repository.findByUsername(username).publishOn(Schedulers.parallel())
        .filter(u -> this.passwordEncoder.matches((String) authentication.getCredentials(),
            u.getPassword()))
        .switchIfEmpty(Mono.error(new BadCredentialsException("Invalid Credentials"))).map(u -> {
