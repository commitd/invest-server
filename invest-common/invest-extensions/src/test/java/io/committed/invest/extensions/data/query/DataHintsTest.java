...
public class DataHintsTest {
  ...
  protected void assertFilter(
      final String db, final String ds, final boolean dup, final DataProvider... expected) {
    final DataHints hints = new DataHints(db, ds, dup);

    final List<DataProvider> filtered =
        hints.filter(Flux.fromIterable(dataProviders)).collectList().block();
