...
public class AbstractMongoCrudDataProviderTest {
  ...
  public void testReplaceStringBsonDocument() {
    ...
    doReturn(Mono.just(new Document())).when(documentCollection).findOneAndReplace(ArgumentMatchers.eq(filter),
    ...
    verify(documentCollection).findOneAndReplace(ArgumentMatchers.eq(filter), ArgumentMatchers.eq(replacement),
        ArgumentMatchers.any(FindOneAndReplaceOptions.class));
  }
  ...
  public void testReplaceStringBsonSClassOfS() {
    ...
    final Document filter = new Document();
    doReturn(Mono.just(new Document())).when(replacementCollection).findOneAndReplace(ArgumentMatchers.eq(filter),
        ArgumentMatchers.eq(r),
        ArgumentMatchers.any(FindOneAndReplaceOptions.class));

    dp.replace("collectionName", filter, r, Replacement.class);

    verify(replacementCollection).findOneAndReplace(
  ...
  public void testUpdate() {
    ...
    doReturn(Mono.just(new Document())).when(documentCollection).findOneAndUpdate(ArgumentMatchers.eq(filter),
        ArgumentMatchers.eq(update));

    dp.update("collectionName", new Document(), new Document());

    verify(documentCollection).findOneAndUpdate(ArgumentMatchers.eq(filter), ArgumentMatchers.eq(update));
  }
  ...
  public void testDelete() {
    ...
    doReturn(Mono.just(deleteResult)).when(documentCollection).deleteMany(ArgumentMatchers.eq(filter));
  ...
  public static class StubMongoCrudDataProvider extends AbstractMongoCrudDataProvider<String, Object> {
